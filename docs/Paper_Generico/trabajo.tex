...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
              minimum height=1cm, minimum width=3cm, fill=blue!8},
  qbox/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
              minimum height=1cm, minimum width=3cm, fill=orange!15},
  envbox/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
...
\node[qbox, below=1.4cm of fm] (kernel) {Kernel de\\Fidelidad};
\node[box, left=of kernel] (svm) {SVM\\Clásica};
\node[box, left=of svm] (out) {Clasificación\\Benigno / Ataque};

% ===== Entornos de ejecución (debajo del kernel) =====
\node[envbox, below=1cm of kernel, fill=green!10] (sim) {Simulador\\clásico (ideal)};
\node[envbox, right=0.6cm of sim, fill=red!10] (hw) {Hardware real\\SpinQ (ruido)};

% ===== Flechas fila superior =====
\draw[arr] (data) -- (pre);
\draw[arr] (pre) -- (fm);

% ===== Flecha vertical fm -> kernel =====
\draw[arr] (fm) -- (kernel);
...
% ===== Flechas entornos -> kernel =====
\draw[arr] (sim.north) -- (kernel.south);
\draw[arr] (hw.north) -- (kernel.south);

% ===== Recuadro punteado: núcleo cuántico =====
\begin{scope}[on background layer]
\node[draw, dashed, rounded corners=3pt, inner sep=7pt,
      fit=(fm)(kernel)(sim)(hw), fill=gray!5] (qcore) {};
\end{scope}
\node[font=\scriptsize\itshape, anchor=south] at (qcore.north) {Núcleo cuántico (Qiskit)};

\end{tikzpicture}%
}
\caption{Arquitectura general del \textit{pipeline} QSVM. El flujo de
datos avanza de izquierda a derecha en la fila superior y retorna en la
inferior. El núcleo cuántico puede ejecutarse sobre un simulador clásico
o sobre hardware físico antes de la clasificación.}
